...
*/
    ...
    do {
            ...
            for i in descId {
                ...
                var docsName: String = ""
                let rows = try
                            self.database.prepare(self.docTable.filter(docId == i[self.docId]))
                for row in rows {
    ...
    @IBAction func docSelectMenu(_ sender: UIButton) {
        ...
        depDown.selectionAction = { [weak self] (index: Int, item: String) in
            ...
            self!.timeOutlet.setTitle("Choose hour", for: .normal)
            let components = self!.appointmentDate.calendar.dateComponents([.year, .month, .day, .weekday],
                                                                           from: self!.appointmentDate.date)
        ...
        if nameSurname.text == "" {
            ...
            do {
                ...
                createAlert(title: "Success",
                            ...
                            "Please be at the hospital 15 minutes before your appointment time.")
            }
            catch {
                print(error)
            }
